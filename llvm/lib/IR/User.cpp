...
void User::allocHungoffUses(unsigned N, bool IsPhi) {
  assert(getOperandInfo().HasHungOffUses && "alloc must have hung off uses");
...
void User::growHungoffUses(unsigned NewNumUses, bool IsPhi) {
  assert(getOperandInfo().HasHungOffUses && "realloc must have hung off uses");
...
MutableArrayRef<uint8_t> User::getDescriptor() {
  assert(getOperandInfo().HasDescriptor && "Don't call otherwise!");
  assert(!getOperandInfo().HasHungOffUses && "Invariant!");
...
void *User::allocateFixedOperandUser(size_t Size, unsigned Us,
                                     unsigned DescBytes) {
  assert(Us < (1u << OperandInfo::Contents::NumUserOperandsBits) &&
         "Too many operands");
  ...
  uint8_t *Storage = static_cast<uint8_t *>(::operator new(
      Size + sizeof(OperandInfo) + sizeof(Use) * Us + DescBytesToAllocate));
  Use *Start = reinterpret_cast<Use *>(Storage + DescBytesToAllocate);
  Use *End = Start + Us;
  OperandInfo *OI = reinterpret_cast<OperandInfo *>(End);
  User *Obj = reinterpret_cast<User *>(OI + 1);
  Obj->getOperandInfo().NumUserOperands = Us;
  Obj->getOperandInfo().HasHungOffUses = false;
  Obj->getOperandInfo().HasDescriptor = DescBytes != 0;
  assert(&(OI->contents) == &Obj->getOperandInfo() &&
         "getOperandInfo() is returning the wrong location");
...
void *User::operator new(size_t Size) {
  ...
  void *Storage = ::operator new(Size + sizeof(OperandInfo) + sizeof(Use *));
  Use **HungOffOperandList = static_cast<Use **>(Storage);
  OperandInfo *OI = reinterpret_cast<OperandInfo *>(HungOffOperandList + 1);
  User *Obj = reinterpret_cast<User *>(OI + 1);
  Obj->getOperandInfo().NumUserOperands = 0;
  Obj->getOperandInfo().HasHungOffUses = true;
  Obj->getOperandInfo().HasDescriptor = false;
  assert(&(OI->contents) == &Obj->getOperandInfo() &&
         "getOperandInfo() is returning the wrong location");
...
LLVM_NO_SANITIZE_MEMORY_ATTRIBUTE void User::operator delete(void *Usr) {
  ...
  if (Obj->getOperandInfo().HasHungOffUses) {
    assert(!Obj->getOperandInfo().HasDescriptor && "not supported!");
  ...
  } else if (Obj->getOperandInfo().HasDescriptor) {
